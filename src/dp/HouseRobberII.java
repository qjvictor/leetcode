...
public class HouseRobberII {
	...
	private int helper(int[] nums, boolean robFirst){
		int pre = robFirst ? nums[0] : 0;
		int cur = robFirst ? Math.max(nums[0], nums[1]) : nums[1];
		for (int i = 2; i < nums.length; i++) {
			if (i != nums.length - 1 || (i == nums.length - 1 && !robFirst)) {
	...
	public static void main(String[] args){
		...
		System.out.println(hr.rob(new int[]{2,3,8,7,10,11,8,6,3,12}));
	}
}
